...
By default, this element will establish a connection with the server at the same
...
If the `auto` attribute is set, the element will try to establish a connection
immediately or whenever the URL changes.
...
`,
  ...
  properties: {
    ...
    /**
     * The URL of the websocket server to connect to. By default, it is the same
     * hostname as the page, at port 9090.
  ...
  connect: function() {
    ...
    var that = this;
    this.ros.on('connection', function() { that.fire('connection'); });
    this.ros.on('error', function(error) { that.fire('error', error); });
    this.ros.on('close', function() { that.fire('close'); });
  },
  ...
  /**
   * The `connection` event is fired when the connection to the websocket server
   * is established.
   *
   * @event connection
   */
  ...
  /**
   * The `close` event is fired when the connection to the websocket server is
   * closed.
   *
   * @event close
   */
